...
A refit runs on your labels, and writing them is the work. The tool takes a built store, query vectors from `examples/eval_query_embed.rs`, and two files you write yourself: the queries, and which chunks of your vault answer each one. Nothing ships those and nothing can — they are a judgement about your own notes. The shipped fit rests on 33 labeled answers against 1228 labeled non-answers, and a fit is worth what its labels are worth. Two things move the floor out from under a fit. The keyword half measures itself against each query's own upper bound, but the semantic half is a raw cosine, and where that sits depends both on the embedder and on how long the notes it is scoring are. So a vault whose notes are much shorter than the ones a fit was built on can put the floor inside the band its own correct answers score in. The symptom is an abstention on a query that retrieved everything: `--explain` shows the lanes hitting and nothing coming back. Refit, or lower `[calibrated] floor`, when that is what you see.
