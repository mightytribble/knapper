...
- `config.rs` — loads `~/.engraph/config.toml` and `vault.toml`, merges CLI args, provides `data_dir()`. Includes `intelligence: Option<bool>`, `max_chunks_per_file` + `group_by` (retrieval granularity), `[models]` section for model overrides, `[obsidian]` section (CLI path, enabled flag), and `[agents]` section (registered AI agent names). `Config::save()` writes back to disk.
...
- Unit tests in each module (`cargo test --lib`) — 517 tests, no network required
