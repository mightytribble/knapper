...
- `graph.rs` — vault graph agent. Extracts wikilink targets, expands search results by following graph connections 1-2 hops. Relevance filtering via FTS5 term check and shared tags; the matching chunk becomes the result's section, since fusion is chunk-keyed
...
- `store.rs` — SQLite persistence. Tables: `meta`, `files` (with docid, created_by), `chunks` (with vector BLOBs, `text` — the whole chunk, which the reranker scores — `snippet`, the leading 200 characters for display and *derived from `text`* by `insert_chunk` so callers pass one string rather than two, and `seq`, the chunk's ordinal within its file), `chunks_fts` (FTS5, holding each chunk's full text for keyword search; `chunks.text` is the copy that can be fetched by key, since FTS5's `file_id`/`chunk_seq` are `UNINDEXED`), `edges` (vault graph), `tombstones`, `tag_registry`, `folder_centroids`, `placement_corrections`, `link_skiplist` (reserved), `llm_cache` (orchestrator result cache), `cli_events` (audit log for CLI operations). `chunks_vec` virtual table (sqlite-vec) for KNN search, declared at the embedding model's own width — it is not created until something establishes that width, since `Store::init` runs before any model loads. `ensure_embedding_dim()` sizes it at index time and discards the index if the width changed; `verify_embedding_dim()` guards the read and write paths. Enhanced `resolve_file()` with fuzzy Levenshtein matching as final fallback
- `prefix.rs` — contextual embedding prefix (#2). `DocContext::from_file` parses a document's identity (frontmatter `name` else filename stem, path, aliases, tags); `embed_texts` prepends it plus the chunk's ancestor heading path to the text handed to the embedder. Storage, snippets and FTS keep the raw chunk, so the prefix cannot leak into results. `PrefixConfig` is off by default and each component (`path`/`heading`/`tags`/`aliases`) is separately switchable — see `eval/probes.md` for why.
- `indexer.rs` — orchestrates vault walking (via `ignore` crate for `.gitignore` support), diffing, chunking, embedding, writes to store + sqlite-vec + FTS5, vault graph edge building (wikilinks + people detection), and folder centroid computation. Exposes `index_file`, `remove_file`, `rename_file` as public per-file functions. `run_index_shared` accepts external store/embedder for watcher FullRescan. Reconciles the store's embedding width against the model before writing anything, forcing a full rebuild when it changed.
...
- `search.rs` — hybrid search orchestrator. `search_with_intelligence()` runs the full pipeline: orchestrate (intent + expansions, then `ensure_original` so the user's own query is searched for whatever the model returned — issue #23) → 5-lane RRF retrieval (semantic + FTS5 + graph + reranker + temporal) per expansion → two-pass RRF fusion. `search_internal()` is a thin wrapper without intelligence models. Lanes dedup on `(file_id, chunk_seq)` and cap each document's share (`max_chunks_per_file`, default 3); `group_by = "file"` collapses to one result per document. Adaptive lane weights per query intent including temporal (1.5 weight for time-aware queries). Results display normalized confidence percentages (0-100%) instead of raw RRF scores. `--explain` prints the retrieval step as well as the fusion step: every query actually run, its FTS MATCH expression, and per-lane hit counts. `rerank_documents()` fetches each candidate's whole chunk, truncates it to `[rerank] max_document_chars` (before prepending any title, so the title cannot be what gets cut) and logs the assembled size at DEBUG — `RUST_LOG=engraph::search=debug` is how `eval/sweep-rerank-chars.py` reads the character counts back.
...
- Unit tests in each module (`cargo test --lib`) — 574 tests, no network required
