...
mod tests {
    ...
    /// A server over two notes tagged differently and linked to neither, so a
    /// scoped call has one note to admit and one to exclude (#64).
    fn tagged_server() -> (tempfile::TempDir, super::EngraphServer) {
        use std::sync::Arc;
        use tokio::sync::Mutex;
        ...
        let server = super::EngraphServer {
            ...
            read_only: false,
            max_chunks_per_file: crate::config::default_max_chunks_per_file(),
            group_by: crate::config::GroupBy::File,
            top_n: crate::config::Config::default().top_n,
            rerank: crate::config::RerankConfig::default(),
            ranking: crate::config::RankingConfig::default(),
            lane_weights: crate::config::LaneWeights::default(),
    ...
    fn topic_params(all: Vec<String>) -> crate::params::Topic {
        crate::params::Topic {
    ...
    /// The paths a topic bundle names, in the order the bundle holds them.
    fn bundle_paths(result: &rmcp::model::CallToolResult) -> Vec<String> {
    ...
    #[tokio::test]
    async fn a_topic_scope_keeps_the_bundle_inside_the_tagged_notes() {
        // #64. The tool takes the four operators `search` takes and hands them
        // to the same pipeline, so a scoped bundle is assembled from the
        // scoped notes alone.
        let (_tmp, server) = tagged_server();

        let unscoped = server
            .topic(super::Parameters(topic_params(vec![])))
            .await
            .unwrap();
        let paths = bundle_paths(&unscoped);
        assert!(
            paths.contains(&"wolf.md".to_string()),
            "unscoped, both notes answer: {paths:?}"
        );

        let scoped = server
            .topic(super::Parameters(topic_params(vec![
                "type/undead".to_string(),
            ])))
            .await
            .unwrap();
        let paths = bundle_paths(&scoped);
        assert!(!paths.is_empty(), "the tagged note still answers");
        for p in &paths {
            assert_eq!(p, "wight.md", "an out-of-scope note reached the bundle");
        }
    }

    #[tokio::test]
    async fn a_topic_scope_naming_no_tag_fails_the_call() {
        // The caller's own text named nothing, so the call fails rather than
        // answering from the whole vault (#64).
        let (_tmp, server) = tagged_server();
        let err = server
            .topic(super::Parameters(topic_params(vec![
                "type/undeed".to_string(),
            ])))
            .await
            .expect_err("a scope naming no tag must fail the call");
        assert!(err.message.contains("no such tag"), "got: {}", err.message);
    }
